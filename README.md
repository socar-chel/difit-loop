...
<img src="docs/loop.png" alt="한 라운드 — 에이전트: ① 코드 쓰고 스스로 점검 → ② 리뷰 창을 연다 → ④ 코멘트 읽고 고쳐서 커밋 → ⑤ 답글을 새 코드 위에 다시 붙임 → ⑦ draft PR 만들기. 사람: ③ 브라우저에서 코드 보며 코멘트를 단다 → 코멘트 달았어요 → ⑥ 새 버전에서 답을 확인한다 → 더 묻거나 OK" width="100%">
